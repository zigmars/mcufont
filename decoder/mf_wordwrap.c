...
struct linelen_s {
    ...
    int16_t width;  /* Total length of all words + whitespace on the line in
                       pixels. */
...
static void tune_lines(struct linelen_s* current, struct linelen_s* previous,
    ...
    prevw2 = previous->width - previous->last_word.word -
             previous->last_word.space - previous->last_word_2.space;
    ...
    if (delta1 > delta2 && curw2 <= max_width) {
        ...
        while (chars--)
            mf_rewind(&current->start);
    }
}

void mf_wordwrap(const struct mf_font_s* font, int16_t width, mf_str text,
                 mf_line_callback_t callback, void* state) {
    ...
    while (*text) {
        ...
        if (full || current.linebreak) {
            ...
            if (previous.chars) {
                ...
                if (!callback(previous.start, previous.chars, state)) return;
            }
    ...
    if (current.chars) callback(current.start, current.chars, state);
}
...
void mf_wordwrap(const struct mf_font_s* font, int16_t width, mf_str text,
    ...
    while (*text) {
        ...
        if (!callback(linestart, cc_prev, state)) return;
