...
static uint8_t scaled_render_character(const struct mf_font_s* font, int16_t x0,
                                       int16_t y0, mf_char character,
    ...
    basewidth = sfont->basefont->render_character(
        sfont->basefont, 0, 0, character, scaled_pixel_callback, &rstate);

    return sfont->x_scale * basewidth;
}

void mf_scale_font(struct mf_scaledfont_s* newfont,
                   const struct mf_font_s* basefont, uint8_t x_scale,
                   uint8_t y_scale) {
