...
static const struct mf_bwfont_char_range_s*
find_char_range(const struct mf_bwfont_s* font, uint16_t character,
                uint16_t* index_ret) {
    unsigned i, index;
...
static uint8_t get_width(const struct mf_bwfont_char_range_s* r,
                         uint16_t index) {
    if (r->width) return r->width + r->offset_x;
    return r->glyph_widths[index];
}

static uint8_t render_char(const struct mf_bwfont_char_range_s* r, int16_t x0,
                           int16_t y0, uint16_t index,
...
uint8_t mf_bwfont_render_character(const struct mf_font_s* font, int16_t x0,
                                   int16_t y0, uint16_t character,
                                   mf_pixel_callback_t callback, void* state) {
