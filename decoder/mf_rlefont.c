...
static void write_rle_dictentry(const struct mf_rlefont_s* font,
                                struct renderstate_r* rstate, uint8_t index) {
    uint16_t offset = pgm_read_word(font->dictionary_offsets + index);
    uint16_t length =
        pgm_read_word(font->dictionary_offsets + index + 1) - offset;
    uint16_t i;
...
static void write_bin_codeword(const struct mf_rlefont_s* font,
                               struct renderstate_r* rstate, uint8_t code) {
    ...
    if (runlen) write_pixels(rstate, runlen, 255);
}
...
uint8_t mf_rlefont_render_character(const struct mf_font_s* font, int16_t x0,
    ...
    while (rstate.y < rstate.y_end) {
        write_glyph_codeword((struct mf_rlefont_s*)font, &rstate,
                             pgm_read_byte(p++));
    }
