...
static int16_t mf_round_to_tab(const struct mf_font_s* font, int16_t x0,
                               int16_t x) {
    int16_t tabw, dx;
...
static int16_t mf_round_to_prev_tab(const struct mf_font_s* font, int16_t x0,
                                    int16_t x) {
    int16_t tabw, dx;
...
int16_t mf_get_string_width(const struct mf_font_s* font, mf_str text,
    ...
    while (count-- && *text) {
        ...
        if (kern && c1 != 0) result += mf_compute_kerning(font, c1, c2);
...
static void render_left(const struct mf_font_s* font, int16_t x0, int16_t y0,
    ...
    while (count--) {
        ...
        if (c1 != 0) x += mf_compute_kerning(font, c1, c2);
...
void mf_render_aligned(const struct mf_font_s* font, int16_t x0, int16_t y0,
                       enum mf_align_t align, mf_str text, uint16_t count,
                       mf_character_callback_t callback, void* state) {
...
static void render_right(const struct mf_font_s* font, int16_t x0, int16_t y0,
                         mf_str text, uint16_t count,
                         mf_character_callback_t callback, void* state) {
    int16_t x;
    uint16_t i;
    ...
    for (i = 0; i < count; i++) {
        ...
        /* Apply kerning */
        if (c2 != 0) x -= mf_compute_kerning(font, c1, c2);
...
void mf_render_justified(const struct mf_font_s* font, int16_t x0, int16_t y0,
                         int16_t width, mf_str text, uint16_t count,
                         mf_character_callback_t callback, void* state) {
    mf_render_aligned(font, x0, y0, MF_ALIGN_LEFT, text, count, callback,
                      state);
}
