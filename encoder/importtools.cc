...
void crop_glyphs(std::vector<DataFile::glyphentry_t>& glyphtable,
    ...
    for (DataFile::glyphentry_t& glyph : glyphtable) {
        if (glyph.data.size() == 0) continue; // Dummy glyph
    ...
    if (bbox.right < bbox.left) return; // There were no glyphs
...
void detect_flags(const std::vector<DataFile::glyphentry_t>& glyphtable,
    ...
    if (is_monospace) fontinfo.flags |= DataFile::FLAG_MONOSPACE;
    ...
    if (is_bw) fontinfo.flags |= DataFile::FLAG_BW;
}
