...
static status_t cmd_import_ttf(const std::vector<std::string>& args) {
    if (args.size() != 3 && args.size() != 4) return STATUS_INVALID;

    std::string src = args.at(1);
    int size = std::stoi(args.at(2));
    bool bw = (args.size() == 4 && args.at(3) == "bw");
    std::string dest =
        strip_extension(src) + std::to_string(size) + (bw ? "bw" : "") + ".dat";
    std::ifstream infile(src);
    ...
    if (!save_dat(dest, f.get())) return STATUS_ERROR;

    std::cout << "Done: " << f->GetGlyphCount() << " unique glyphs."
              << std::endl;
    return STATUS_OK;
}

static status_t cmd_import_bdf(const std::vector<std::string>& args) {
    if (args.size() != 2) return STATUS_INVALID;
...
static status_t cmd_filter(const std::vector<std::string>& args) {
    if (args.size() < 3) return STATUS_INVALID;
    ...
    for (size_t i = 2; i < args.size(); i++) {
        ...
        if (pos == std::string::npos) {
            if (s == "gb2312") {
                allowed.insert(&gb2312_in_ucs2_codetable[0],
                               &gb2312_in_ucs2_codetable
                                   [sizeof(gb2312_in_ucs2_codetable) /
    ...
    std::cout << "Font originally had " << f->GetGlyphCount() << " glyphs."
              << std::endl;
    ...
    for (size_t i = 0; i < f->GetGlyphCount(); i++) {
        ...
        if (g.chars.size()) { newglyphs.push_back(g); }
    }
    ...
    std::cout << "After filtering, " << f->GetGlyphCount() << " glyphs remain."
              << std::endl;

    if (!save_dat(src, f.get())) return STATUS_ERROR;
...
static status_t cmd_show_glyph(const std::vector<std::string>& args) {
    ...
    std::cout << "Chars: ";
    for (int c : f->GetGlyphEntry(index).chars)
        std::cout << c << " ";
    std::cout << std::endl;
...
static status_t cmd_rlefont_export(const std::vector<std::string>& args) {
    if (args.size() != 2 && args.size() != 3) return STATUS_INVALID;

    std::string src = args.at(1);
    std::string dst =
        (args.size() == 2) ? strip_extension(src) + ".c" : args.at(2);
    std::unique_ptr<DataFile> f = load_dat(src);
...
static status_t cmd_rlefont_size(const std::vector<std::string>& args) {
    ...
    std::cout << "Uncompressed size: "
              << f->GetGlyphCount() * f->GetFontInfo().max_width *
                     f->GetFontInfo().max_height / 2
              << " bytes" << std::endl;
    std::cout << "Compressed size:   " << size << " bytes" << std::endl;
    std::cout << "Bytes per glyph:   " << size / f->GetGlyphCount()
              << std::endl;
    return STATUS_OK;
}
...
static status_t cmd_rlefont_optimize(const std::vector<std::string>& args) {
    ...
    std::cout << "Results are saved automatically after each iteration."
              << std::endl;

    int limit = 100;
    if (args.size() == 3) { limit = std::stoi(args.at(2)); }
    ...
    while (!limit || i < limit) {
        ...
        std::cout << "iteration " << i << ", size " << newsize
                  << " bytes, speed " << bytes_per_min << " B/min" << std::endl;
...
static const char* usage_msg =
    ...
    "   import_ttf <ttffile> <size> [bw]     Import a .ttf font into a data "
    "file.\n"
    "   import_bdf <bdffile>                 Import a .bdf font into a data "
    "file.\n"
    "\n"
    "Commands for inspecting and editing data files:\n"
    "   filter <datfile> <range> ...         Remove everything except "
    "specified characters.\n"
    "   show_glyph <datfile> <index>         Show the glyph at index.\n"
    "\n"
    "Commands specific to rlefont format:\n"
    "   rlefont_size <datfile>               Check the encoded size of the "
    "data file.\n"
    "   rlefont_optimize <datfile>           Perform an optimization pass on "
    "the data file.\n"
    "   rlefont_export <datfile> [outfile]   Export to .c source code.\n"
    "   rlefont_show_encoded <datfile>       Show the encoded data for "
    "debugging.\n"
    "\n"
...
int main(int argc, char** argv) {
    ...
    if (status == STATUS_INVALID) { std::cout << usage_msg << std::endl; }
