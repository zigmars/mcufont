...
static size_t prefix_length(const DataFile::pixels_t& pixels, size_t pos) {
    ...
    size_t count = 1;
    while (pos + count < pixels.size() && pixels.at(pos + count) == pixel) {
        count++;
    }
    return count;
}
...
static encoded_font_t::rlestring_t
encode_rle(const DataFile::pixels_t& pixels) {
    ...
    while (pos < pixels.size()) {
        ...
        if (pixel == 0) {
            ...
            // encode using RLE_64ZEROS, and then whatever remains with
            // RLE_ZEROS.
            while (count >= 64) {
            ...
            if (count) { result.push_back(RLE_ZEROS | count); }
        } else if (pixel == 15) {
            // Encode ones.
            while (count) {
...
class DictTreeNode {
  public:
    DictTreeNode()
        : m_index(-1), m_ref(false), m_length(0), m_child0(nullptr),
          m_child15(nullptr), m_suffix(nullptr) {}
    ...
    void SetChild(uint8_t p, DictTreeNode* child) {
        if (p == 0)
            ...
            if (!m_children) { m_children.reset(new DictTreeNode*[14]()); }
...
static void fill_tree_suffixes(DictTreeNode* root, DictTreeNode* subtree,
    ...
    for (size_t i = 1; i < entry.size(); i++) {
        DictTreeNode* node =
            find_tree_node(entry.begin() + i, entry.end(), root);
        if (node) {
...
static DictTreeNode*
construct_tree(const std::vector<DataFile::dictentry_t>& dictionary,
...
static encoded_font_t::refstring_t
encode_ref_slow(const DataFile::pixels_t& pixels, const DictTreeNode* root,
                bool is_glyph) {
    // Chain of encodings. Each entry in this array corresponds to a position
    // in the pixel string.
    std::unique_ptr<encoding_link_t[]> chain(
        new encoding_link_t[pixels.size() + 1]);
    ...
    for (size_t pos = 0; pos < pixels.size(); pos++) {
        ...
        while (suffix != root) {
            if (suffix->GetIndex() >= 0 && (is_glyph || !suffix->GetRef())) {
                ...
                if (link.length < chain[pos + 1].length) chain[pos + 1] = link;
            }
...
// Returns number of pixels encoded, and index is set to the dictionary
// reference.
...
static encoded_font_t::refstring_t
encode_ref_fast(const DataFile::pixels_t& pixels, const DictTreeNode* tree,
                bool is_glyph) {
    encoded_font_t::refstring_t result;
    ...
    if (i < pixels.size()) result.push_back(REF_FILLZEROS);
...
size_t
estimate_tree_node_count(const std::vector<DataFile::dictentry_t>& dict) {
...
std::unique_ptr<encoded_font_t> encode_font(const DataFile& datafile,
    ...
    for (const DataFile::dictentry_t& d : sorted_dict) {
        if (d.replacement.size() == 0) {
            ...
            result->ref_dictionary.push_back(
                encode_ref(d.replacement, tree, false, fast));
        } else {
    ...
    if (!fast) {
        for (size_t i = 0; i < datafile.GetGlyphCount(); i++) {
            ...
            if (*decoded != datafile.GetGlyphEntry(i).data) {
                auto iter =
                    std::mismatch(decoded->begin(), decoded->end(),
                                  datafile.GetGlyphEntry(i).data.begin());
                size_t pos = iter.first - decoded->begin();
                throw std::logic_error(
                    "verification of glyph " + std::to_string(i) +
...
size_t get_encoded_size(const encoded_font_t& encoded) {
    ...
    for (const encoded_font_t::rlestring_t& r : encoded.rle_dictionary) {
        ...
        if (r.size() != 0) total += 2; // Offset table entry
    }
...
std::unique_ptr<DataFile::pixels_t>
decode_glyph(const encoded_font_t& encoded,
    ...
    for (uint8_t ref : refstring) {
        ...
        } else if (ref - DICT_START - encoded.rle_dictionary.size() <
                   encoded.ref_dictionary.size()) {
            size_t index = ref - DICT_START - encoded.rle_dictionary.size();
            std::unique_ptr<DataFile::pixels_t> part = decode_glyph(
                encoded, encoded.ref_dictionary.at(index), fontinfo);
...
std::unique_ptr<DataFile::pixels_t>
decode_glyph(const encoded_font_t& encoded, size_t index,
             const DataFile::fontinfo_t& fontinfo) {
