...
std::unique_ptr<DataFile::pixels_t> random_substring(const DataFile& datafile,
                                                     rnd_t& rnd) {
    std::uniform_int_distribution<size_t> dist1(0,
                                                datafile.GetGlyphCount() - 1);
...
void optimize_worst(DataFile& datafile, size_t& size, rnd_t& rnd,
                    bool verbose) {
    ...
    if (newsize < size) {
        ...
        if (verbose)
            std::cout << "optimize_worst: replaced " << worst << " score "
                      << d.score << std::endl;
    }
}
...
void optimize_any(DataFile& datafile, size_t& size, rnd_t& rnd, bool verbose) {
    ...
    if (newsize < size) {
        ...
        if (verbose)
            std::cout << "optimize_any: replaced " << index << " score "
                      << d.score << std::endl;
    }
}
...
void optimize_expand(DataFile& datafile, size_t& size, rnd_t& rnd, bool verbose,
    ...
    if (newsize < size) {
        ...
        if (verbose)
            std::cout << "optimize_expand: expanded " << index << " by "
                      << count << " pixels, score " << d.score << std::endl;
    }
}
...
void optimize_trim(DataFile& datafile, size_t& size, rnd_t& rnd, bool verbose) {
    ...
    std::uniform_int_distribution<size_t> dist2(
        0, std::min((int)d.replacement.size() / 2, 5));
    size_t start = dist2(rnd);
    size_t end = dist2(rnd);

    if (start) {
        d.replacement.erase(d.replacement.begin(),
                            d.replacement.begin() + start);
    }
    ...
    if (newsize < size) {
        ...
        if (verbose)
            std::cout << "optimize_trim: trimmed " << index << " by " << start
                      << " pixels from start and " << end
                      << " pixels from end, score " << d.score << std::endl;
    }
}

// Switch random dictionary entry to use ref encoding or back to rle.
void optimize_refdict(DataFile& datafile, size_t& size, rnd_t& rnd,
                      bool verbose) {
    DataFile trial = datafile;
    std::uniform_int_distribution<size_t> dist1(0,
                                                DataFile::dictionarysize - 1);
    ...
    if (newsize < size) {
        ...
        if (verbose)
            std::cout << "optimize_refdict: switched " << index << " to "
                      << (d.ref_encode ? "ref" : "RLE") << ", score " << d.score
                      << std::endl;
    }
}

// Combine two random dictionary entries.
void optimize_combine(DataFile& datafile, size_t& size, rnd_t& rnd,
                      bool verbose) {
    DataFile trial = datafile;
    std::uniform_int_distribution<size_t> dist1(0,
                                                DataFile::dictionarysize - 1);
    ...
    const DataFile::pixels_t& part1 =
        datafile.GetDictionaryEntry(index1).replacement;
    const DataFile::pixels_t& part2 =
        datafile.GetDictionaryEntry(index2).replacement;
    ...
    if (newsize < size) {
        ...
        if (verbose)
            std::cout << "optimize_combine: combined " << index1 << " and "
                      << index2 << " to replace " << worst << ", score "
                      << d.score << std::endl;
    }
}

// Pick a random part of an encoded glyph and encode it as a ref dict.
void optimize_encpart(DataFile& datafile, size_t& size, rnd_t& rnd,
                      bool verbose) {
    ...
    if (newsize < size) {
        ...
        if (verbose)
            std::cout << "optimize_encpart: replaced " << worst << " score "
                      << d.score << std::endl;
    }
}
...
// behaviour.
void optimize_parallel(DataFile& datafile, size_t& size, rnd_t& rnd,
                       bool verbose, int num_threads = 4) {
    ...
    for (int i = 0; i < num_threads; i++) {
        threads.emplace_back(new std::thread(
            optimize_pass, std::ref(datafiles.at(i)), std::ref(sizes.at(i)),
            std::ref(rnds.at(i)), verbose));
    }
    ...
    for (size_t i = 0; i < DataFile::dictionarysize; i++) {
        ...
        if (d.score > 0) {
            ...
            if (verbose && d.replacement.size() != 0)
                std::cout << "update_scores: dropped " << i << " score "
                          << -d.score << std::endl;
        }
    }
}
...
void init_dictionary(DataFile& datafile) {
    ...
    if (datafile.GetGlyphCount() == 0) return;
