...
#define FT_ERROR_START_LIST                                                    \
    static const std::map<FT_Error, std::string> ft_errors {
...
std::unique_ptr<DataFile> LoadFreetype(std::istream& file, int size, bool bw) {
    ...
    auto topx = [size, u_per_em](int s) {
        return (s * size + u_per_em / 2) / u_per_em;
    };

    fontinfo.name = std::string(face->family_name) + " " +
                    std::string(face->style_name) + " " + std::to_string(size);
    ...
    while (gindex) {
        try {
            ...
            std::cerr << "Skipping glyph " << gindex << ": " << e.what()
                      << std::endl;
    ...
    std::unique_ptr<DataFile> result(
        new DataFile(dictionary, glyphtable, fontinfo));
    return result;
}
