...
static void encode_dictionary(std::ostream& out, const std::string& name,
    ...
    write_const_table(out, data, "uint8_t",
                      "mf_rlefont_" + name + "_dictionary_data", 1);
    write_const_table(out, offsets, "uint16_t",
                      "mf_rlefont_" + name + "_dictionary_offsets", 1, 4);
}
...
static void encode_character_range(std::ostream& out, const std::string& name,
    ...
    write_const_table(
        out, data, "uint8_t",
        "mf_rlefont_" + name + "_glyph_data_" + std::to_string(range_index), 1);
    write_const_table(out, offsets, "uint16_t",
                      "mf_rlefont_" + name + "_glyph_offsets_" +
...
void write_source(std::ostream& out, std::string name,
    ...
    out << "#ifndef MF_RLEFONT_VERSION_" << RLEFONT_FORMAT_VERSION
        << "_SUPPORTED" << std::endl;
    ...
    out << "static const struct mf_rlefont_char_range_s mf_rlefont_" << name
        << "_char_ranges[] = {" << std::endl;
    for (size_t i = 0; i < ranges.size(); i++) {
        out << "    {" << ranges.at(i).first_char << ", "
            << ranges.at(i).char_count << ", mf_rlefont_" << name
            << "_glyph_offsets_" << i << ", mf_rlefont_" << name
            << "_glyph_data_" << i << "}," << std::endl;
    }
    out << "};" << std::endl;
    out << std::endl;

    // Pull it all together in the rlefont_s structure.
    out << "const struct mf_rlefont_s mf_rlefont_" << name << " = {"
        << std::endl;
    out << "    {" << std::endl;
    out << "    "
    ...
    out << "    " << datafile.GetFontInfo().line_height << ", /* line height */"
        << std::endl;
    out << "    " << datafile.GetFontInfo().flags << ", /* flags */"
        << std::endl;
    ...
    out << "    " << encoded->rle_dictionary.size() << ", /* rle dict count */"
        << std::endl;
    out << "    "
        << encoded->ref_dictionary.size() + encoded->rle_dictionary.size()
        << ", /* total dict count */" << std::endl;
