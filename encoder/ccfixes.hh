...
class thread {
  ...
  public:
    ...
    explicit thread(Function&& f, Args&&... args) {
        ...
        mHandle =
            (HANDLE)_beginthreadex(NULL, 0, threadfunc<Call>, (LPVOID)call, 0,
                                   (unsigned*)&(mThreadId.mId));
    }
    template <class Call> static unsigned int __stdcall threadfunc(void* arg) {
    ...
    void join() {
        ...
        if (!joinable()) throw system_error(EINVAL, generic_category());
    ...
    void detach() {
        if (!joinable()) throw system_error();
        mHandle = _STD_THREAD_INVALID_HANDLE;
        mThreadId.clear();
    }
};
