...
std::string filename_to_identifier(std::string name) {
    ...
    for (pos = 0; pos < name.size(); pos++) {
        if (!isalnum(name.at(pos))) name.at(pos) = '_';
    }

    return name;
}

// Write a vector of integers as line-wrapped hex/integer data for initializing
// const array.
void wordwrap_vector(std::ostream& out, const std::vector<unsigned>& data,
    ...
    for (size_t i = 0; i < data.size(); i++) {
        if (i % values_per_column == 0 && i != 0) out << std::endl << prefix;
...
void write_const_table(std::ostream& out, const std::vector<unsigned>& data,
                       ...
                       const std::string& tablename, char flg_is_data,
                       size_t width) {
    out << "static const " << datatype << " " << tablename;
    out << "[" << data.size() << "]" << ((flg_is_data) ? " PROGMEM " : " ")
        << "= {" << std::endl;
...
int select_fallback_char(const DataFile& datafile) {
    ...
    if (chars.count(0xFFFD)) return 0xFFFD; // Unicode replacement character

    if (chars.count(0)) return 0; // Used by many BDF fonts as replacement char
...
std::vector<char_range_t>
compute_char_ranges(const DataFile& datafile,
                    std::function<size_t(size_t)> get_encoded_glyph_size,
                    size_t maximum_size, size_t minimum_gap) {
